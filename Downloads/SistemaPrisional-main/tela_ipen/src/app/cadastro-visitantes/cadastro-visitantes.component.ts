...
export class CadastroVisitantesComponent implements OnInit {
  ...
  isVelhoVirilSelected(): boolean {
    return !!this.visitante && this.visitante.prisioneiroId != null && Number(this.visitante.prisioneiroId) === this.velhoVirilId;
  }
  ...
  cadastrarVisitante() {
    ...
    if (!this.visitante.eAdvogado && Number(this.visitante.prisioneiroId) === this.velhoVirilId) {
    ...
    const payload = {
      ...
      prisioneiroId: Number(this.visitante.prisioneiroId)
    };
